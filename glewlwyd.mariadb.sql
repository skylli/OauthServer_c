...
DROP TABLE IF EXISTS `g_controllist`;
DROP TABLE IF EXISTS `g_category`;
DROP TABLE IF EXISTS `g_device_state`;
DROP TABLE IF EXISTS `g_function`;
DROP TABLE IF EXISTS `g_category_function`;
DROP TABLE IF EXISTS `g_factory`;
DROP TABLE IF EXISTS 'g_user_resource'
-- ----------- --
-- Data tables --
-- ----------- --
...
CREATE TABLE `g_user` (
  ...
  `gu_uuid` VARCHAR(128) NOT NULL UNIQUE, 
...
CREATE TABLE `g_category` (
  `gctg_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `gctg_name` VARCHAR(128) NOT NULL UNIQUE, 
  `gctg_type` VARCHAR(128) DEFAULT 'NONE', 
  `gctg_description` VARCHAR(512) DEFAULT ''
);

CREATE INDEX `i_g_category_name` ON `g_category`(`ctg_name`);

CREATE TABLE `g_resource` (
  `gr_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `gr_name` VARCHAR(128) DEFAULT '',
  `gr_uuid` VARCHAR(128) NOT NULL UNIQUE,
  `gr_aeskey`  VARCHAR(128) NOT NULL,
  `gr_register_time` timestamp NOT NULL default CURRENT_TIMESTAMP,
  `gr_factory`  VARCHAR(32) DEFAULT '',
  `gr_description` VARCHAR(512) DEFAULT '',
  `gr_category_name` VARCHAR(128) DEFAULT 'none'
  -- 该 device 访问的服务器 ip.
  `gr_server_ip` VARCHAR(128) DEFAULT ''
  -- `gr_uri` VARCHAR(512)
);
CREATE INDEX `i_g_resource_name` ON `g_resource`(`gr_name`);
CREATE INDEX `i_gr_factory_name` ON `g_resource`(`gr_factory`);
CREATE INDEX `i_gr_category_name` ON `g_resource`(`gr_category`);
...
-- control list 
CREATE TABLE `g_controllist` (
  `gcd_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `gr_id` INT(11) NOT NULL,
  `gu_id` INT(11) NOT NULL,
  FOREIGN KEY(`gr_id`) REFERENCES `g_resource`(`gr_id`) ON DELETE CASCADE,
  FOREIGN KEY(`gu_id`) REFERENCES `g_user`(`gu_id`) ON DELETE CASCADE
...
-- function list.
CREATE TABLE `g_factory` (
  `gfc_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `gfc_name` VARCHAR(128) DEFAULT '',
  `gfc_description` VARCHAR(512) DEFAULT ''
);
-- function list.
CREATE TABLE `g_function` (
  `gf_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `gf_name` VARCHAR(128) NOT NULL UNIQUE,
  `gf_description` VARCHAR(512) DEFAULT ''
);
-- the reference of categosry and functions .
CREATE TABLE `g_category_function` (
  `gcf_id` INT(11) PRIMARY KEY AUTO_INCREMENT,
  `gctg_id` INT(11) NOT NULL,
  `gf_id` INT(11) NOT NULL,
  FOREIGN KEY(`gctg_id`) REFERENCES `g_category`(`gctg_id`) ON DELETE CASCADE,
...
-- init database 
-- build category  
INSERT INTO g_category (gctg_name,gctg_description) VALUES ("socket","turn on and off");
INSERT INTO g_category (gctg_name,gctg_description) VALUES ("christmas_trees","led light things");

-- add device 
INSERT INTO g_resource (gr_uuid,gr_aes_key,gr_factory,gr_description) VALUES ("000c2957fbe4", "0000000000000000","dongguanghumeng","smarthome device");
INSERT INTO g_resource (gr_name,gr_uuid,gr_aes_key,gr_factory,gr_description) VALUES ("pxmalion 3d printer","000c2957fbe4", "0000000000000000","dongguanghumeng","smarthome device");
-- add user 
-- add factory
INSERT INTO g_factory (gfc_name,gfc_description) VALUES ("evalogik","smart home devices: socket/led/christmas trees");

--add function 
INSERT INTO g_function (gf_name,gf_description) VALUES ("turnon","pin");
-- add g_category_function
INSERT INTO g_category_function (gctg_id,gf_id) VALUES (1,1);
INSERT INTO g_category_function (gctg_id,gf_id) VALUES (1,2);
