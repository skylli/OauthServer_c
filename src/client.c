...
json_t * client_check(struct config_elements * config, const char * client_id, const char * client_id_header, const char * client_password_header, const char * redirect_uri, const int auth_type) {
  ...
  if (client_id != NULL) {
    ...
    char *p = json_dumps(j_client,JSON_COMPACT);
        if(p){
            y_log_message(Y_LOG_LEVEL_DEBUG,"dev is : %s\n",p);
            free(p);
            p = NULL;
        }
      ...
      if (j_res == NULL) {
...
#if 0
        // todo: we only auth the host,avoid it redirect to other web.
        if (auth_type == GLEWLWYD_AUHORIZATION_TYPE_AUTHORIZATION_CODE || auth_type == GLEWLWYD_AUHORIZATION_TYPE_CODE || auth_type == GLEWLWYD_AUHORIZATION_TYPE_IMPLICIT) {
          ...
          if (redirect_uri != NULL) {
            ...
            y_log_message(Y_LOG_LEVEL_DEBUG, ">>> find redirct url");
...
#else
        y_log_message(Y_LOG_LEVEL_WARNING, ">>> redirect_uri = %s redirect_uri_allowed = %d auth_type_allowed =%d",redirect_uri,redirect_uri_allowed,auth_type_allowed);

        // return unauth which no rediret url.
        if (redirect_uri == NULL) 
            redirect_uri_allowed = 0;
        else{
...
#endif // end of the redirct host check.
        // Final check, is everything ok?
        if (!redirect_uri_allowed || !auth_type_allowed) {            
            
          y_log_message(Y_LOG_LEVEL_WARNING, ">>> auth_type_allowed = %d redirect_uri_allowed = %d",auth_type_allowed,redirect_uri_allowed);
          y_log_message(Y_LOG_LEVEL_WARNING, ">>> G_ERROR_UNAUTHORIZED");
    ...
    y_log_message(Y_LOG_LEVEL_WARNING, ">>> G_ERROR_PARAM");
