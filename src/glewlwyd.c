...
int main (int argc, char ** argv) {
  ...
  // add user resource
  ulfius_add_endpoint_by_val(config->instance, "POST", config->url_prefix, "/profile/resource/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_glewlwyd_add_resource_by_user, (void*)config);
  // return user resource    
   ulfius_add_endpoint_by_val(config->instance, "GET", config->url_prefix, "/profile/resource/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_glewlwyd_get_resource_by_user, (void*)config);
  // delete resource
  ulfius_add_endpoint_by_val(config->instance, "DELETE", config->url_prefix, "/profile/resource/:resource", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_glewlwyd_delete_resource, (void*)config);
  // add new user
  ulfius_add_endpoint_by_val(config->instance, "POST", config->url_prefix, "/newuser/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_glewlwyd_add_user, (void*)config);
  // update user 
  ...
  if (config->use_secure_connection) {
    ...
    y_log_message(Y_LOG_LEVEL_INFO,"key path = %s",config->secure_connection_key_file);
    y_log_message(Y_LOG_LEVEL_INFO,"key file = %s",key_file);
    y_log_message(Y_LOG_LEVEL_INFO,"key path = %s",config->secure_connection_pem_file);
    y_log_message(Y_LOG_LEVEL_INFO,"pem_file = %s",pem_file);
    
    if (key_file != NULL && pem_file != NULL) {
      res = ulfius_start_secure_framework(config->instance, key_file, pem_file);
            // todo log
      y_log_message(Y_LOG_LEVEL_INFO,"ulfius_start_secure_framework res = %d",res);
    } else {
        
      y_log_message(Y_LOG_LEVEL_INFO," Error in input parameters ");
      res = U_ERROR_PARAMS;
    }
    o_free(key_file);
    o_free(pem_file);
    ...
    y_log_message(Y_LOG_LEVEL_INFO,"ulfius_start_framework res = %d",res);
  }
  if (res == U_OK) {
    // Wait until stop signal is broadcasted
    
    y_log_message(Y_LOG_LEVEL_INFO," todo >>>> Wait until stop signal is broadcasted!!");
    ...
    y_log_message(Y_LOG_LEVEL_INFO," todo >>>> pthread end!!");
  } else {
    y_log_message(Y_LOG_LEVEL_ERROR, " Error starting glewlwyd webserver   res = %d",res);
    exit_server(&config, GLEWLWYD_ERROR);
  }
  if (pthread_mutex_destroy(&global_handler_close_lock) ||
    ...
    y_log_message(Y_LOG_LEVEL_ERROR, "Error destroying global_handler_close_lock or global_handler_close_cond");
      ...
      y_log_message(Y_LOG_LEVEL_INFO," todo >>>> exit_server !!");
...
char * get_file_content(const char * file_path) {
  ...
  if (f) {
    ...
    y_log_message(Y_LOG_LEVEL_INFO, " file %s open success  buffer = %p!!",file_path,buffer);
    ...
    if (buffer) {
      ...
      buffer[length] = '\0';
        ...
        y_log_message(Y_LOG_LEVEL_ERROR, "alloc failt !!");
    }
    ...
    fclose (f);
        ...
        y_log_message(Y_LOG_LEVEL_ERROR, "open file %s failt res = %p !!",file_path,f);
  }
