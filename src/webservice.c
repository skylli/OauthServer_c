...
int callback_glewlwyd_authorization (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (0 == o_strcmp("code", response_type)) {
      y_log_message(Y_LOG_LEVEL_DEBUG,">>> meth = %s \n url : %s",request->http_verb,request->http_url);
      y_log_message(Y_LOG_LEVEL_DEBUG,">>> redirect_uri = %s \n",u_map_get(request->map_url, "redirect_uri"));
    ...
    if (0 == o_strcasecmp("GET", request->http_verb) && is_authorization_type_enabled((struct config_elements *)user_data, GLEWLWYD_AUHORIZATION_TYPE_CODE) == G_OK && u_map_get(request->map_url, "redirect_uri") != NULL) {
      ...
      if (u_map_get(request->map_url, "redirect_uri") != NULL) {
        ...
        y_log_message(Y_LOG_LEVEL_ERROR,">>> 403 error new");
        response->status = 403;
      }
    }
...
int callback_glewlwyd_check_user (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  y_log_message(Y_LOG_LEVEL_INFO, " >>> TOKEN %s",  u_map_get(request->map_header, "Authorization"));
...
int callback_glewlwyd_add_resource (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (j_result != NULL && json_array_size(j_result) == 0) {
    json_t * json_body = ulfius_get_json_body_request(request, NULL);
    Log_dump_jason("request jason ", json_body);
...
#if 0
int callback_glewlwyd_get_resource_by_useruuid (const struct _u_request * request, struct _u_response * response, void * user_data) {
  struct config_elements * config = (struct config_elements *)user_data;
  // get uuid
  
  json_t * json_req_body = ulfius_get_json_body_request(request, NULL);
  
  json_t * j_result = get_resource_by_useruuid(config, json_req_body);
  if (check_result_value(j_result, G_OK)) {
    json_t * json_body = json_copy(json_object_get(j_result, "resource"));
    ulfius_set_json_body_response(response, 200, json_body);
    json_decref(json_body);
    ...
    y_log_message(Y_LOG_LEVEL_ERROR, "callback_glewlwyd_get_list_resource - Error getting resource by uuid");
    response->status = 500;
  }
  json_decref(j_result);
  json_decref(json_req_body);
 ...
 * User session endpoints
 */
int callback_glewlwyd_get_resource_by_user (const struct _u_request * request, struct _u_response * response, void * user_data) {
  struct config_elements * config = (struct config_elements *)user_data;
  json_t * j_session = NULL, * j_user = NULL;
  ...
  json_t *j_result = get_resource_by_user(config, json_req_body,
    json_string_value(json_object_get(json_object_get(j_session, "grants"), "username")));

    y_log_message(Y_LOG_LEVEL_DEBUG, " jason %p",j_result);
  ...
  y_log_message(Y_LOG_LEVEL_DEBUG, "32");
    response->status = 404;
  } else {
  ...
  y_log_message(Y_LOG_LEVEL_DEBUG, "33");
  json_decref(j_result);
  json_decref(json_req_body);
  
  y_log_message(Y_LOG_LEVEL_DEBUG, "34");
  json_decref(j_session);
  return U_CALLBACK_CONTINUE;
}
int callback_glewlwyd_add_resource_by_user (const struct _u_request * request, struct _u_response * response, void * user_data) {

  struct config_elements * config = (struct config_elements *)user_data;
  json_t * j_session = NULL, * j_user = NULL;
  
  j_session = session_or_access_token_check(config, u_map_get(request->map_cookie, config->session_key),
                         u_map_get(request->map_header, "Authorization"));
  if (check_result_value(j_session, G_OK)) {

    json_t * json_body = ulfius_get_json_body_request(request, NULL);
    json_t * j_result = is_resource_valid(config, json_body, 1);
    
    Log_dump_jason("j_result jason ", j_result);
    if (j_result != NULL && json_array_size(j_result) == 0) {
      json_t * json_body = ulfius_get_json_body_request(request, NULL);
      
      Log_dump_jason("request jason ", json_body);
      if (add_resource_by_username(config, json_body,
        json_string_value(json_object_get(json_object_get(j_session, "grants"), "username"))) != G_OK) {
 ...
 * User session endpoints
 */
int callback_glewlwyd_get_user_session_profile__ (const struct _u_request * request, struct _u_response * response, void * user_data) {
  struct config_elements * config = (struct config_elements *)user_data;
  json_t * j_session = NULL, * j_user = NULL;
