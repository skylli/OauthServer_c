...
// resource and user relate
#define GLEWLWYD_TABLE_USER_RESOURCE            "g_user_resource"
...
// debug dump jason struct 
#define Log_dump_jason(str,json)  do{ char *p = json_dumps(json,JSON_COMPACT);\
                                         if(p){\
                                             y_log_message(Y_LOG_LEVEL_DEBUG, "%s %s",str,p);\
                                             free(p);\
                                             p = NULL;\
                                         }\
                                    }while(0)
