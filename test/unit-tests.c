...
/**
 * just for debug by sky
 */
void print_request(struct _u_request * req) {
    printf(" ############ request {\n");
    if(req->http_protocol)
    ...
    printf("\t http_verb : %d\n",req->check_server_certificate);    
    printf("\t http_url : %ld\n",req->timeout);

    if(req->client_address)
    printf("\t http_url : %d.%d.%d.%d\n",req->client_address[0],req->client_address[1],req->client_address[2],req->client_address[3]);
    if(req->auth_basic_user)
    printf("\t auth_basic_user : %s\n",req->auth_basic_user);    
    if(req->auth_basic_password)
    printf("\t auth_basic_password : %s\n",req->auth_basic_password);
