  ...
  if (res == U_OK) {
    ...
    if (ulfius_send_http_request(&scope_req, NULL) != U_OK) {
      ...
      if (ulfius_send_http_request(&user_req, &code_resp) != U_OK) {
        ...
        // todo 
        print_response(&code_resp);
        y_log_message(Y_LOG_LEVEL_DEBUG, " >>> respon redircetion location %s", u_map_get(code_resp.map_header, "Location"));
        y_log_message(Y_LOG_LEVEL_DEBUG, " >>> access token %s",code);
        // end
